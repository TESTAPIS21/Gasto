...
class PurchaseInvoice(BuyingController):
	def __init__(self, *args, **kwargs):
		...
		self.status_updater = [
			{
				"source_dt": "Purchase Invoices Item",
	...
	def validate(self):
		...
		self.clear_unallocated_advances("Purchase Invoices Advance", "advances")
			...
			if (
				...
				if self.update_stock and item.warehouse and (not item.from_warehouse):
					...
					if item.purchase_receipt:
						...
						# This is done in cases when Purchase Invoices is created before Purchase Receipt
						if (
							for_validate and item.expense_account and item.expense_account != stock_not_billed_account
						):
							msg = _(
							...
							msg += "<br>"
							msg += _(
								"This is done to handle accounting for cases when Purchase Receipt is created after Purchase Invoices"
							)
							frappe.msgprint(msg, title=_("Expense Head Changed"))
			...
			self.status_updater.append(
				{
					...
					"extra_cond": """ and exists(select name from `tabPurchase Invoices`
					where name=`tabPurchase Invoices Item`.parent and update_stock = 1)""",
				}
			)
				...
				self.status_updater.append(
					{
						...
						"overflow_type": "receipt",
						"extra_cond": """ and exists (select name from `tabPurchase Invoices`
						where name=`tabPurchase Invoices Item`.parent and update_stock=1 and is_return=1)""",
					}
				)
					...
					if provisional_accounting_for_non_stock_items:
						if item.purchase_receipt:
							...
							if expense_booked_in_pr:
								# Intentionally passing Purchase Invoices item to handle partial billing
	...
	def validate_supplier_invoice(self):
		...
		if self.bill_no:
			if cint(frappe.db.get_single_value("Accounts Settings", "check_supplier_invoice_uniqueness")):
				...
				pi = frappe.db.sql(
					"""select name from `tabPurchase Invoices`
					where
						bill_no = %(bill_no)s
				...
				if pi:
					pi = pi[0][0]
					frappe.throw(_("Supplier Invoice No exists in Purchase Invoices {0}").format(pi))
	...
	def get_pr_details_billed_amt(self):
		# Get billed amount based on purchase receipt item reference (pr_detail) in Purchase Invoices

		pr_details_billed_amt = {}
		pr_details = [d.get("pr_detail") for d in self.get("items") if d.get("pr_detail")]
		if pr_details:
			doctype = frappe.qb.DocType("Purchase Invoices Item")
			query = (
...
# to get details of Purchase Invoices/receipt from which this doc was created for exchange rate difference handling
def get_purchase_document_details(doc):
	if doc.doctype == "Purchase Invoices":
		...
		child_doctype = "Purchase Invoices Item"
